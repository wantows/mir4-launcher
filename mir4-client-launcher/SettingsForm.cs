...
namespace Mir_4_Launcher
{
    public partial class SettingsForm : Form
    {
        ...
        public SettingsForm()
        {
            ...
            string saveFilePath = Path.Combine("MirMobile", "MirMobile", "SaveData", "Saved", "SaveGames", "AccountLocalSave1.sav");
            string saveFilePath1 = Path.Combine("MirMobile", "MirMobile", "SaveData", "Saved", "SaveGames", "AccountLocalSave2.sav");

            // Get Both Usernames
            ACID1.Text = GetUsername(saveFilePath);
            ACID2.Text = GetUsername(saveFilePath1);

            // Get Both Passwords
            ACPW1.Text = GetPassword(saveFilePath1);
            ACPW2.Text = GetPassword(saveFilePath);
        }
        ...
        public static string GetUsername(string saveFile)
        {
            try
            {
                if (File.Exists(saveFile))
                {
                    byte[] bytes = File.ReadAllBytes(saveFile); // Read the save file as a Byte Array

                    string accountString = Encoding.ASCII.GetString(bytes, FindByteArray(saveFile, guestID), 64); // Find the GuestID in the Byte array grab and return the next 64 bytes as a string
                    string[] accountUsername = accountString.Split('\0'); //Split the string at each NUL and return the list to a string array
                    return accountUsername[16]; // return the 17th string from the array
                }
                else
                {
        ...
        public static string GetPassword(string saveFile)
        {
            try
            {
                if (File.Exists(saveFile))
                {
                    byte[] bytes = File.ReadAllBytes(saveFile); // Read the save file as a Byte Array
                    string accountString = Encoding.ASCII.GetString(bytes, FindByteArray(saveFile, linkID), 64); // Find the LinkID in the Byte array grab and return the next 64 bytes as a string
                    string[] accountPassword = accountString.Split('\0'); //Split the string at each NUL and return the list to a string array
                    return accountPassword[16]; // return the 17th string from the array
                }
                else
                {
                    return "Error: Unable to locate the Save File"; // return error unable to locate the file
